document.addEventListener('DOMContentLoaded', function () {
    ...
    const loginBtn = document.querySelector('.btn'); // Corrected selector for login button

    loginBtn.addEventListener('click', function () { // Corrected event listener name
        hideKeyboard();
    });

    const virtualKeyboardButtons = document.querySelectorAll('.button');
...
function showKeyboard() {
    document.querySelector('.kbody').classList.add('active');
...
enterBtn.addEventListener('click', () => {
    hideKeyboard();
    document.querySelector('.kbody').classList.remove('active');
});
...
buttons.forEach(btn => {
    btn.addEventListener('click', () => {
        if (btn.innerText !== 'ENTER' && btn.innerText !== 'CLEAR') {
